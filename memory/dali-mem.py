...
def parse():
    ...
    parser.add_argument('--exp_name', default="", type=str,
                        help='named preprocessing time txt')
...
def main():
    ...
    used_mem = print_memory_pycuda()
    with open("mem-b{}-t{}.txt".format(args.batch_size, args.workers), 'a') as f:
        # f.write("batch {} time -> {:.9f}\n".format(i,end_preprocess))
        f.write("{}\t{}\n".format(time.time(), used_mem))
...
def train(train_loader, model, criterion, scaler, optimizer, epoch, decoder_pipe=None):
    ...
    # print("epoch time ->", end_epoch)
    # with open("preproces-time-dali.txt", 'a') as f:
    #     f.write("epoch time -> {:.9f}\n".format(end_epoch))
