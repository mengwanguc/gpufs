...
gpu_count="8"
model="alexnet"
memory_limit=$((17 * 1024 * 1024 * 1024)) # should be ~65%-ish cached of imagenette
batch_size="256"
n_workers="24"
...
mv ./$gpu_type/$model-batch$batch_size.csv ./$gpu_type/$model-$batch_size-batch_size-$n_workers-workers-$gpu_count-gpus-$memory_limit-limit-$usage-usage-$cached-cached-$node_count-nodes-$node_id-id.csv
echo
