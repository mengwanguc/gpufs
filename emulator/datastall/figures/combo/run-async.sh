...
for n_workers in ${workers[@]}; do
    ...
    python main-measure-time-emulator.py --cgroup ../memory/$group_name --use-async True --skip-epochs 1 --emulator-version=1 -j 4 --epoch 2 --workers $n_workers --gpu-count 1 --gpu-type $gpu_type -a $model --batch-size $batch_size --profile-batches -1 $data_path
