...
for n_workers in ${workers[@]}; do
    ...
    cgexec -g memory:$group_name python main-measure-time-emulator.py --use-async True --use-minio True -c $cache_size --skip-epochs 1 --emulator-version=1 -j 4 --epoch 2 --workers $n_workers --gpu-count 1 --gpu-type $gpu_type -a $model --batch-size $batch_size --profile-batches -1 $data_path
    ...
    mv ./$gpu_type/$model-batch$batch_size.csv ./$gpu_type/$model-$batch_size-batch_size-$n_workers-workers-$limit-limit-$usage-usage-$cached-cached-minio_async.csv
    echo
