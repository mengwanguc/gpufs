...
for superbatch in ${superbatch_configs[@]}; do
    ...
    cgexec -g memory:$group_name python main-measure-time-emulator.py --use-async True --super-batch-size $superbatch --prefetch-factor $prefetch_factor --skip-epochs 0 --emulator-version=1 -j 4 --epoch 1 --workers $n_workers --gpu-count 1 --gpu-type $gpu_type -a $model --batch-size $batch_size --profile-batches -1 -f $extension $data_path
