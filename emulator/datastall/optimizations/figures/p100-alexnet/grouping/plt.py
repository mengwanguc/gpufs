...
n_groups = 4
columns = ['use_file_group', 'group_size', 'io', 'cpu2gpu', 'gpu']
schemes = {}
local = {}
...
ax1.set_ylabel('Epoch training time (s)', fontsize=25)
ax1.set_xlabel('File group size', fontsize=25)

plt.xticks(xtics, xlabels)

plt.title("Performance evaluation of file grouping using emulator\nAlexnet on 2 emulated P100 GPUs on HDD with 2 workers\n")

# legend reverse order
handles, labels = plt.gca().get_legend_handles_labels()
ax1.legend(reversed(handles), reversed(labels), markerfirst=False)
