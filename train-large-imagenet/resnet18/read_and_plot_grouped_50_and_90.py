...
with open('resnet18_batch_256_epo_90_ ') as json_file:
...
with open('sequential-groupingresnet18_batch_256_gsize_64_epo_90_ ') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
            batch_top1_top5_size16 = data[k]
with open('sequential-groupingresnet18_batch_256_gsize_64_epo_50_') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
...
print(epoch_range)
plt.plot(epoch_range, batch_top1_top5_size2, label = 'no_grouping', color="blue", linewidth=1)
plt.plot(epoch_range, batch_top1_top5_size8, '--', label = 'rand-perm-grouping', color="green", linewidth=3)
