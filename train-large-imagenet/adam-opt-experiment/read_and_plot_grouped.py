...
with open('resnet18_batch_256_epo_5_ ') as json_file:
...
# with open('sequential-groupingresnet18_batch_256_gsize_64_epo_50_sequential-grouping') as json_file:
#     data = json.load(json_file)
#     for k,v in data.items():
#         if k == current_p:
...
plt.plot(epoch_range, batch_top1_top5_size2, label = 'SGD Optimzer', color="red", linewidth=1)
plt.plot(epoch_range, batch_top1_top5, '--', label = 'Adam Optimizer', color="green", linewidth=3)
...
bat = str(batch_size)
results_dir = ''
sample_file_name = "comparison_opt_resnet18"+"_"+current_p + "_batch_" + bat +"_epo_"+ ep +".png" 
