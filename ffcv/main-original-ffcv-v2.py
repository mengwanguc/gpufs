...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_image_pipeline = [
        ...
        ToTensor(),
        ToDevice(torch.device(f'cuda:{args.gpu}'), non_blocking=True),
        ToTorchImage(),
    ...
    train_loader = Loader('/home/cc/ffcv-imagenet/train_500_0_50_90.ffcv', batch_size=args.batch_size, num_workers=args.workers,
                          order=OrderOption.RANDOM, os_cache=True, drop_last=True,
                          pipelines={'image': train_image_pipeline, 'label': label_pipeline}, seed=0)

    val_loader = Loader('/home/cc/ffcv-imagenet/val_500_0.50_90.ffcv', batch_size=args.batch_size, num_workers=args.workers,
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        print("images1 ->", images, type(images))
        print("target1 ->", target, type(target))
        print("images1 ->", type(images), images.size())
        print("target1 ->", type(target), target.size())
        # tensor = images.type(torch.cuda.HalfTensor)
        # print(type(tensor))
        output = model(images)
        # output = model(images)
