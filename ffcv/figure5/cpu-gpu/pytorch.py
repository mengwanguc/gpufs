...
class DALIGenericIterator(_DaliBaseIterator):
    ...
    def __next__(self):
        ...
        for i in range(self._num_gpus):
            ...
            # print("data_batches ->", data_batches['label'].dtype, type(data_batches['label']), data_batches['label'].shape, data_batches['label'].device)
            # quit()
            ...
            pyt_tensors = dict()
            pyt_tensors['label'] = torch.zeros((256, 1), dtype=torch.int32, device='cuda:0')
            # print("pyt_tensors ->", pyt_tensors['label'].dtype, type(pyt_tensors['label']), pyt_tensors['label'].shape, pyt_tensors['label'].device)
            pyt_tensors['data'] = torch.zeros((256, 3, 224, 224), dtype=torch.float32, device='cuda:0')
            # print("pyt_tensors ->", pyt_tensors['data'].dtype, type(pyt_tensors['data']), pyt_tensors['data'].shape, pyt_tensors['label'].device)

            data_batches[i] = pyt_tensors
            data_batches[i]['label'] = torch.zeros((256, 1), dtype=torch.int32, device='cuda:0')

            data_batches[i]['data'] = torch.zeros((256, 3, 224, 224), dtype=torch.float32, device='cuda:0')
