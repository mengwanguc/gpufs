...
current_title = 'Batch related Validation top 1 Accuracy'
process = '_trainingtop1_'

with open('grouped_model_data/resnet101_batch_'+short_batch+'_gsize_1_epo_50') as json_file:
    ...
    for k,v in data.items():
        if k == current_p:
            ...
            batch_size = data[k]
with open('grouped_model_data/resnet101_batch_'+short_batch+'_gsize_2_epo_50') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
            batch_top1_top5_size2 = data[k]
with open('grouped_model_data/resnet101_batch_'+short_batch+'_gsize_4_epo_50') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
            batch_top1_top5_size4 = data[k]
with open('grouped_model_data/resnet101_batch_'+short_batch+'_gsize_8_epo_50') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
            batch_top1_top5_size8 = data[k]
with open('grouped_model_data/resnet101_batch_'+short_batch+'_gsize_16_epo_50') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
            batch_top1_top5_size16 = data[k]
with open('grouped_model_data/resnet101_batch_'+short_batch+'_gsize_32_epo_50') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
            batch_top1_top5_size32 = data[k]
with open('grouped_model_data/resnet101_batch_'+short_batch+'_gsize_64_epo_50') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
