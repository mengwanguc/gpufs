...
def main():
    ...
    if args.multiprocessing_distributed:
        print("distributed multiprocessiing...")
...
#    traindir = os.path.join(args.data, 'train')
    traindir = "/home/cc/data/mytar/" + str(img_per_tar)
...
#        best_acc1 = max(acc1, best_acc1)
    print('total_train_time: {}\t total_data_time: {}'.format(total_train_time, total_data_time))
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    data_time = 0
    display_time = 0
    move_data_to_gpu_time = 0
    compute_output_time = 0
    compute_loss_time = 0
    optimize_time = 0
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # compute output
        start_compute_output_time = time.time()
        output = model(images)
        compute_output_time += time.time() - start_compute_output_time

        start_compute_loss_time = time.time()
        loss = criterion(output, target)
        compute_loss_time += time.time() - start_compute_loss_time
        ...
        loss.backward()
        optimizer.step()
        optimize_time += time.time() - start_optimize_time

        start_compute_accuracy_time = time.time()
        acc1, acc5 = accuracy(output, target, topk=(1, 5))
        top1.update(acc1[0], images.size(0))
        top5.update(acc5[0], images.size(0))
        accuracy_compute_time += time.time() - start_compute_accuracy_time
    ...
    total_train_time += time.time() - end
