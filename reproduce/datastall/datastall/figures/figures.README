...
2. Run `worker-mem-vary.sh`. This will take a long time (likely tens of minutes per worker/limit pair). The output will be saved to `p100` (e.g., `alexnet-256-batch_size-2-workers-10G-limit-69.4%-usage.csv`). You may want to run this using a [tmux](https://tmuxcheatsheet.com/) session, so that if your connection breaks at some point, the script will continue running.
3. Copy the output to wherever you're running the Jupyter notebook (`DNNStall-figures.ipynb`).
4. Configure the paths and worker/limit settings in the notebook and run the "Worker/Mem Heatmap" section.
