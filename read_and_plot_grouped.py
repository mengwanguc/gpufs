...
current_title = 'Epoch related Average Training top 5 Accuracy'
process = '_trainingtop5_'

folder_data = ['imagenette','imagenette2','imagenette3','imagenette4','imagenette5']
for data_name in folder_data:
    with open(data_name+'/resnet18_batch_'+short_batch+'_gsize_1_epo_50') as json_file:
        ...
        for k,v in data.items():
            if k == current_p:
                ...
                batch_size = data[k]
    with open(data_name+'/resnet18_batch_'+short_batch+'_gsize_2_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size2.append(data[k])
    with open(data_name+'/resnet18_batch_'+short_batch+'_gsize_4_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size4.append(data[k])
    with open(data_name+'/resnet18_batch_'+short_batch+'_gsize_8_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size8.append(data[k])
    with open(data_name+'/resnet18_batch_'+short_batch+'_gsize_16_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size16.append(data[k])
    with open(data_name+'/resnet18_batch_'+short_batch+'_gsize_32_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size32.append(data[k])
    with open(data_name+'/resnet18_batch_'+short_batch+'_gsize_64_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
