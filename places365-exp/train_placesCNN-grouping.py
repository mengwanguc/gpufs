...
parser.add_argument('--img_per_tar', default=64, type=int,
                    help='img per tar')
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.ImageFolder(traindir, transforms.Compose([
            ...
            transforms.ToTensor(),
            normalize,
        ]), is_tar = True, is_async = (is_async == 1),
        is_mytar = True, group_size = img_per_tar),
    ...
    model_data = {'val_top1': val_top1, 'model':strs,
    'args.epochs': args.epochs, 'args.batch_size': args.batch_size}
    bat = str(args.batch_size)
    epo = str(args.epochs)
    txt = strs + "_batch_" + bat + "_epo_" + epo + "_randgrouping"
