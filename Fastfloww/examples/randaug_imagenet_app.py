...
This example code is written 
based on the implementation of AutoAugment linked below:
https://github.com/tensorflow/tpu/pull/772  
"""
...
def apply_matrix_to_single_image(image, matrix):
  ...
  """
    ...
    idx2 = tf.clip_by_value(idx2,
                            -img_dim // 2 + img_dim_even + 1,
                            img_dim // 2)
...
def translate_x(image, pixels, replace):
    ...
    translation_matrix = tf.reshape(
        tf.concat(
            [one, zero, -pixels, zero, one, zero, zero, zero, one],
            axis=0),
        [3, 3])
...
def translate_y(image, pixels, replace):
    ...
    translation_matrix = tf.reshape(
        tf.concat([one, zero, zero, zero, one, -pixels, zero, zero, one],
                  axis=0),
        [3, 3])
...
def my_augment_single(image):
    image = tf.cast(image, tf.uint8)
    return distort_image_with_randaugment(image, 13, 9)
...
class ResNet50V2(ff.FastFlowModel):
    def __init__(self, input_dim, num_classes):
        ...
        outputs = resnet50_v2(x)

        super().__init__(inputs, outputs)

    def __deepcopy__(self, memo=None):
        return ResNet50V2(self.input_dim, self.num_classes)


class RandAugApp(App):
    def __init__(self, args, config):
        super().__init__(args, config)

        self.ds_train = tfds.load(
            'imagenette',
            data_dir=os.path.join(args.data_prefix, ''),
            download=False, split='train', as_supervised=True)
    ...
    def create_dataset(self, num_parallel):
        train_dataset = self.ds_train.shuffle(BATCH_SIZE * 10)
        train_dataset = train_dataset.map(
            lambda x, y: (tf.image.resize(x, (IMAGE_SIZE, IMAGE_SIZE)), y),
            num_parallel_calls=num_parallel, name='prep_begin')
        train_dataset = train_dataset.map(
            lambda x, y: (my_augment_single(x), y),
