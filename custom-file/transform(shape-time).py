...
class Normalize(torch.nn.Module):
    ...
    def forward(self, tensor: Tensor) -> Tensor:
        ...
        start = time.time()
        result = F.normalize(tensor, self.mean, self.std, self.inplace)
        end = time.time() - start
        with open("/home/cc/gpufs/ffcv/normalize-shape.txt", 'a') as f:
            f.write("{}\t{}\n".format(tensor.shape, result.shape))
        
        with open("/home/cc/gpufs/ffcv/normalize-time.txt", 'a') as f:
            f.write("{:.9f}\n".format(end))
        return result
