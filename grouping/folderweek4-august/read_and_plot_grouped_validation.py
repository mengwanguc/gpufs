...
with open('resnet50_batch_256_epo_90_nogrouping') as json_file:
    ...
    for k,v in data.items():
        if k == current_p:
            ...
            batch_size = data[k]
with open('diffclass-grouping_batch_256_gsize_64_epo_90_resnet50') as json_file:
    data = json.load(json_file)
    for k,v in data.items():
        if k == current_p:
...
plt.title("(Model:Resnet50, Batch Size:256, Optimizer:SGD)", fontsize=10)
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.ylim([0,100])
plt.legend()
