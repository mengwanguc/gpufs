...
current_title = 'Epoch related Average Training top 1 Accuracy'
process = '_trainingtop1_'

folder_data = ['imagenette','imagenette2','imagenette3','imagenette4','imagenette5']
for data_name in folder_data:
    with open(data_name+'/resnet101_batch_'+short_batch+'_gsize_1_epo_50') as json_file:
        ...
        for k,v in data.items():
            if k == current_p:
                ...
                batch_size = data[k]
    with open(data_name+'/resnet101_batch_'+short_batch+'_gsize_2_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size2.append(data[k])
    with open(data_name+'/resnet101_batch_'+short_batch+'_gsize_4_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size4.append(data[k])
    with open(data_name+'/resnet101_batch_'+short_batch+'_gsize_8_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size8.append(data[k])
    with open(data_name+'/resnet101_batch_'+short_batch+'_gsize_16_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size16.append(data[k])
    with open(data_name+'/resnet101_batch_'+short_batch+'_gsize_32_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size32.append(data[k])
    with open(data_name+'/resnet101_batch_'+short_batch+'_gsize_64_epo_50') as json_file:
        data = json.load(json_file)
        for k,v in data.items():
            if k == current_p:
                batch_top1_top5_size64.append(data[k])
...
plt.plot(epoch_range, np.array([sum(x) for x in zip(*batch_top1_top5)])/5, label = 'groupsize_1')
plt.plot(epoch_range, np.array([sum(x) for x in zip(*batch_top1_top5_size2)])/5, label = 'groupsize_2')
plt.plot(epoch_range, np.array([sum(x) for x in zip(*batch_top1_top5_size4)])/5, label = 'groupsize_4')
plt.plot(epoch_range, np.array([sum(x) for x in zip(*batch_top1_top5_size8)])/5, label = 'groupsize_8')
